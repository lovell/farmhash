...
(() => {
  ...
  [100, 1000, 10000].forEach((length) => {
    ...
    new Benchmark.Suite()
      ...
      .add('murmurhash3+seed', () => {
        murmurhash3.murmur32Sync(input, seed);
      })
      .add('farmhash-hash32-string', () => {
        farmhash.hash32(input);
      })
      .add('farmhash-hash32-buffer', () => {
        farmhash.hash32(inputBuffer);
      })
      .add('farmhash-hash32+seed-string', () => {
        farmhash.hash32WithSeed(input, seed);
      })
      .add('farmhash-hash32+seed-buffer', () => {
        farmhash.hash32WithSeed(inputBuffer, seed);
      })
      .add('farmhash-fingerprint32-string', () => {
        farmhash.fingerprint32(input);
      })
      .add('farmhash-fingerprint32-buffer', () => {
        farmhash.fingerprint32(inputBuffer);
      })
      .add('farmhash-fingerprint64-string', () => {
        farmhash.fingerprint64(input);
      })
      .add('farmhash-fingerprint64-buffer', () => {
        farmhash.fingerprint64(inputBuffer);
      })
      .add('farmhash-hash64-string', () => {
        farmhash.hash64(input);
      })
      .add('farmhash-hash64-buffer', () => {
        farmhash.hash64(inputBuffer);
      })
      .add('farmhash-hash64+seed-string', () => {
        farmhash.hash64WithSeed(input, seed);
      })
      .add('farmhash-hash64+seed-buffer', () => {
        farmhash.hash64WithSeed(inputBuffer, seed);
      })
      .add('farmhash-hash64+seeds-string', () => {
        farmhash.hash64WithSeeds(input, seed, seed2);
      })
      .add('farmhash-hash64+seeds-buffer', () => {
