...
NAN_METHOD(Hash64WithSeed) {
  NanScope();
  string input = *String::Utf8Value(args[0]->ToString());
  uint64_t seed = static_cast<uint64_t>(args[1]->Int32Value());
...
NAN_METHOD(Hash64WithSeeds) {
  NanScope();
  string input = *String::Utf8Value(args[0]->ToString());
  uint64_t seed1 = static_cast<uint64_t>(args[1]->Int32Value());
  uint64_t seed2 = static_cast<uint64_t>(args[2]->Int32Value());
